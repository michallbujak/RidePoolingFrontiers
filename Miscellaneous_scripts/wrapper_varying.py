...
if __name__ == "__main__":
    ...
    pbar = tqdm(total=len(np.arange(0, 0.42, 0.03)))
    for prob in np.arange(0, 0.42, 0.03):
        ...
        # output[batch_size] = {
        #     "VehHourTrav_ns": [t["exmas"]["res"]["VehHourTrav_ns"] for t in dotmaps_list_results],
        #     "VehHourTrav": [t["exmas"]["res"]["VehHourTrav"] for t in dotmaps_list_results],
        #     "Time_saved": [t["exmas"]["res"]["VehHourTrav_ns"] -
        #                    t["exmas"]["res"]["VehHourTrav"]
        #                    for t in dotmaps_list_results],
        #     "Time_saved_relative": [(t["exmas"]["res"]["VehHourTrav_ns"] -
        #                    t["exmas"]["res"]["VehHourTrav"])/t["exmas"]["res"]["VehHourTrav_ns"]
        #                    for t in dotmaps_list_results],
        #     "shared_ratio": [t["exmas"]["res"]["shared_ratio"] for t in dotmaps_list_results],
        #     "batch_size": [len(t["exmas"]["requests"]) for t in dotmaps_list_results]
        # }
    ...
    # with open("critical_mass_data3.pickle", 'wb') as file:
    #     pickle.dump(output, file)

    with open("sensitivity_classes3.pickle", 'wb') as file:
        pickle.dump(output, file)
