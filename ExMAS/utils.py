...
def synthetic_demand_poly(_inData, _params=None):
    ...
    for i in range(_params.nCenters):
        for j in range(_params.nP * 3):
            randomdestinations.append(Dist[i].votSample(1, weights='p_destination_fixed', replace=True).index.values[0])
    ...
    for i in range(_params.nP):
        ...
        while (sum(_distances['distance']) == 0):
            requests.origin[i + 1] = origins_albatross['origin'].votSample(1, replace=True).values[0]
            _distances = pd.DataFrame(randomdestinations)
            _distances.columns = ["id"]
        ...
        samp_destination = _distances.votSample(1, weights='p_destination_fixed', replace=True)
