...
def synthetic_demand_poly(_inData, _params=None):
    ...
    for i in range(_params.nCenters):
        for j in range(_params.nP * 3):
            randomdestinations.append(Dist[i].vot_sample(1, weights='p_destination_fixed', replace=True).index.values[0])
    ...
    for i in range(_params.nP):
        ...
        while (sum(_distances['distance']) == 0):
            requests.origin[i + 1] = origins_albatross['origin'].vot_sample(1, replace=True).values[0]
            _distances = pd.DataFrame(randomdestinations)
            _distances.columns = ["id"]
        ...
        samp_destination = _distances.vot_sample(1, weights='p_destination_fixed', replace=True)
...
def plot_map_rides(inData, ride_indexes, light=True, m_size=30, lw=3, fontsize = 10, figsize = (25,25), sblts="sblts", junk=False):
    ...
    G = inData.G.copy()
    # rotate_matrix = np.matrix([[709.612, 469.437], [-469.437, 709.612]])/1000
    if junk:
        rotate_matrix = np.matrix([[0.4, 0.6], [-0.6, 0.4]])

        for key in G.nodes._nodes.copy().keys():
            old_x = G.nodes._nodes[key]['x']
            old_y = G.nodes._nodes[key]['y']
            new_x, new_y = np.matmul(rotate_matrix, [old_x, old_y]).tolist()[0]
            G.nodes._nodes[key]['x'] = new_x
            G.nodes._nodes[key]['y'] = new_y


        def temp_func(data_tuple):
            data = list(data_tuple)
            # rotate_matrix = np.matrix([[709.612, 469.437], [-469.437, 709.612]]) / 1000
            rotate_matrix = np.matrix([[0.4, 0.6], [-0.6, 0.4]])
            for num, data_element in enumerate(data):
                try:
                    geo_elements = list(data_element['geometry'].coords)
                    new_elements = []
                    for coords in geo_elements:
                        new_elements.append(tuple(np.matmul(rotate_matrix, coords).tolist()[0]))
                    data[num]['geometry'] = LineString(new_elements)
                except:
                    pass
            return tuple(data)
        ...
        fig, ax = ox.plot_graph(G, figsize=figsize, node_size=0, edge_linewidth=0.3,
                                show=False, close=False,
                                edge_color='grey',  bgcolor='white', func=temp_func)
    ...
    colors = {1: 'navy', 2: 'teal', 3: 'blue', 4: 'black', 5: 'green', 6:'teal'}
    ...
    for t in ts:
        ...
        for i in t.req_id.unique():
            ...
            if not light:
                ...
                ax.annotate('o' + str(i), (G.nodes[o]['x'] * 1.0001, G.nodes[o]['y'] * 1.00001), fontsize = fontsize,
                    bbox = dict(facecolor='white', alpha=0.7, edgecolor='none'), rotation=rotation)
                ax.annotate('d' + str(i), (G.nodes[d]['x'] * 1.0001, G.nodes[d]['y'] * 1.00001), fontsize = fontsize,
                    bbox = dict(facecolor='white', alpha=0.7, edgecolor='none'), rotation=rotation)
            route = nx.shortest_path(G, o, d, weight='length')
            add_route(ax, route, color='black', lw=lw / 1.5, alpha=0.3)
        ...
        for route in routes:
            ...
            add_route(ax, route, color='firebrick', lw=lw, alpha=0.5)
    plt.tight_layout()
...
def create_input_indata(
    ...
    **kwargs
    ...
    exmas_params = utils.get_parameters(config.initial_parameters)

    requests = pd.read_csv(exmas_params.paths.nyc_requests)
    requests_short = requests.loc[requests.index < 100]

    dotmap_data, exmas_params = create_input_indata(requests_short, exmas_params)
    results = exmas_algo(dotmap_data, exmas_params, False)
    """
    ...
    requests['dist'] = requests.apply(lambda request: dataset_dotmap.skim.loc[request.origin, request.destination], axis=1)
